...
import { LayoutGrid, Maximize, Download, Search, X, Table2, Network, CheckCircle2, AlertTriangle, BookOpen } from 'lucide-react';
import useAppStore from '../../store/useAppStore';
import { exportReport } from '../../engine/exporter';
import DeParaModal from '../Settings/DeParaModal';

export default function GraphToolbar({ onFitView, onAutoLayout }) {
  const [isDeParaOpen, setIsDeParaOpen] = useState(false);
  const {
    viewMode,
    setViewMode,
    searchQuery,
  ...
  const exactCount = matches.filter(m => m.pass <= 3).length;
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <button
              ...
              onClick={() => setIsDeParaOpen(true)}
              title="Dicionário de regras De-Para aprendidas"
            >
              <BookOpen size={13} /> Memória De-Para
            </button>
